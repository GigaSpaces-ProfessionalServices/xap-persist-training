...
public class UserDAO extends BaseDAO {
	...
	public UserDAO() {
		// "user" is a reserved word in PostgreSQL; use a non-reserved table name instead.
		tableName = "app_user";
	}
	...
	protected void deleteItem(Object obj) throws SQLException{
		...
		User user = (User) obj;
		String sql = "DELETE FROM app_user where userAccountId=?";
		ptmt = conn.prepareStatement(sql);
		ptmt.setInt(1,user.getUserAccountId());
		int res = ptmt.executeUpdate();
	}
	...
	protected boolean checkIfObjectExists(Object obj) throws SQLException {
		...
		User user = (User) obj;
		ptmt = conn.prepareStatement("select * from app_user where userAccountId=?");
		ptmt.setInt(1,user.getUserAccountId());
	...
	protected String getCreateTableSQL() throws SQLException{
		...
		return "CREATE TABLE app_user (userAccountId integer NOT NULL, "
